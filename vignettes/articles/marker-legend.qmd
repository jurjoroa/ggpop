...
value is a bookkeeping label there, not a switch. `swatch`/`line`/`point`/`text` are
...
When colour labels belong beside small square keys, use `legend_spec()` to
describe the grid, named colours, and typed symbols. `legend_render()` places
the keys in columns and fits the border after all content is assembled. The
specification keeps its content for related legend variants.

```{r}
#| label: fig-legend-external-keys
#| message: false
#| warning: false
#| fig-width: 9
#| fig-height: 2
...
Use `legend_subset()` to make a smaller legend from the same content. It keeps
the source specification intact and renumbers the selected grid rows. Add a
symbol row with `legend_add_key()`, then render that variant so its border is
measured around the final labels:

```{r}
#| label: legend-variants
#| code-fold: false
