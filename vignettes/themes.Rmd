...
df_t <- data.frame(
  ...
  icon = rep(c("circle", "square"), each = 20)
)
```
...
All three themes accept additional `theme()` layers. Apply them after the
theme call and before `scale_legend_icon()` if used.

```{r custom}
ggplot() +
  geom_pop(data = df_t, aes(icon = icon, color = grp), size = 2, dpi = 72) +
  scale_color_manual(values = c(A = "#1E88E5", B = "#E53935")) +
  theme_pop(base_size = 11) +
  theme(legend.position = "bottom") +
  labs(title = "Custom legend position", color = NULL)
```
