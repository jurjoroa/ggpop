...
## Example 7: HIV CEA — Dual Icon and Dominance Legend

  

Cost-effectiveness plane for HIV screening strategies (Paltiel et
al. 2006, *Ann Intern Med*). Icon legend (screening frequency) and
dominance status legend are driven by separate dummy layers so neither
bleeds into the other. The efficient frontier line connects all
...
suppressWarnings(
  ggplot(df_cea, aes(x = effect, y = cost, icon = icon_col, color = status)) +
    geom_line(data = df_cea %>% filter(status == "ND") %>% arrange(effect),
              ...
              color = "#06D6A0", linewidth = 1, alpha = 0.6, inherit.aes = FALSE) +
    geom_icon_point(size = 3, dpi = 120, show.legend = FALSE) +
    ...
    theme(legend.position = "bottom", legend.box = "vertical",
          panel.grid.minor = element_blank()) +
    scale_legend_icon(size = 6, which = "HIV Screening") +
...
![](examples-geom-icon-point_files/figure-html/hiv-cea-1.png)
